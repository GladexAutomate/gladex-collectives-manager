...
import { RefreshCw, CheckCircle2, Package, TrendingUp, ClipboardList } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { cn } from '@/lib/utils';

const STAGE_LABELS = {
  4:  'Stage 4 — Internal Documentation',
  8:  'Stage 8 — Reservation & Slot Holding',
  10: 'Stage 10 — Booking Confirmation',
  11: 'Stage 11 — Documentation',
  12: 'Stage 12 — Pre-Departure Coordination',
  15: 'Stage 15 — Post-Trip Evaluation & Client Feedback',
};
...
export default function AdminOperations() {
  ...
  const loadData = async () => {
    try {
      ...
      setAdminTasks(tasksArr.filter(t => t.department === 'admin'));
    } catch (e) {
      console.error('AdminOperations loadData error:', e);
    }
    setLoading(false);
  };
  ...
  const doneTasks  = adminTasks.filter(t => t.status === 'done' || t.status === 'completed').length;
    ...
    <div className="space-y-5 pb-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Checklist progress · Internal Docs · Slot Holding · Booking Confirmation · Pre-Departure · Post-Trip
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          { label: 'Total Tasks',        value: totalTasks,     icon: ClipboardList, color: '#a78bfa' },
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-card rounded-2xl border border-border">
          <ClipboardList className="w-12 h-12 mx-auto mb-4" style={{ color: 'rgba(139,92,246,0.3)' }} />
          <h3 className="font-semibold text-foreground mb-1">No admin tasks yet</h3>
          <p className="text-sm text-muted-foreground">Admin checklist tasks will appear here once created in the Workflow module.</p>
        </div>
      ) : (
        <div className="space-y-3">
          <div className="flex items-center gap-3">
            <ClipboardList className="w-4 h-4" style={{ color: '#a78bfa' }} />
            <h3 className="text-base font-bold text-foreground">Admin Checklist Progress</h3>
